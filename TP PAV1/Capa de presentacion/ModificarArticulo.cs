...
namespace TP_PAV1
{
    public partial class ModificarArticulo : Form
    {
        ...
        private bool ActualizarArticulo(Articulo a)
        {
            string cadenaConexion = System.Configuration.ConfigurationManager.AppSettings["cadenaTP1"];
            SqlConnection cn = new SqlConnection(cadenaConexion);
            ...
            {
                SqlCommand cmd = new SqlCommand();
                string consulta = "UPDATE articulos SET nombre_articulo = @articulo WHERE id_articulo like @id_articulo ";
                cmd.Parameters.Clear();
